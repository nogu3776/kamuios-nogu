...
const PROMPT_LOG_ROOT = path.join(PROJECT_ROOT, 'logs', 'showcase');
const PROMPT_LOG_DIR = path.join(PROMPT_LOG_ROOT, 'prompt-generator');
const LEGACY_PROMPT_LOG_DIR = path.join(PROJECT_ROOT, 'logs', 'prompt-generator');
...
async function ensurePromptLogDir() {
  if (PROMPT_LOG_DIR === LEGACY_PROMPT_LOG_DIR) {
    await ensureDir(PROMPT_LOG_DIR);
    return;
  }

  try {
    const legacyStat = await fsPromises.stat(LEGACY_PROMPT_LOG_DIR);
    if (legacyStat && legacyStat.isDirectory()) {
      const targetStat = await fsPromises.stat(PROMPT_LOG_DIR).catch(() => null);
      if (!targetStat) {
        await fsPromises.mkdir(PROMPT_LOG_ROOT, { recursive: true });
        try {
          await fsPromises.rename(LEGACY_PROMPT_LOG_DIR, PROMPT_LOG_DIR);
          return;
        } catch (err) {
          if (err && !['ENOENT', 'EEXIST', 'ENOTEMPTY', 'EXDEV'].includes(err.code)) {
            console.warn('[PromptGenerator] legacy log migration skipped', err.message);
          }
        }
      }
    }
    ...
    if (err && err.code !== 'ENOENT') {
      console.warn('[PromptGenerator] legacy log directory check failed', err.message);
    }
  }
