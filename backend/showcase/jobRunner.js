...
const { getServerMeta } = require('../services/mcpRegistry');
...
const SHOWCASE_SUBDIR = process.env.MCP_SHOWCASE_SUBDIR || 'showcase';
...
async function runMcpJob({
  ...
  try {
    ...
    if (Array.isArray(media) && media.length) {
      ...
      try {
        const { uploads, logs: mediaLogs } = await prepareMediaUploads(media, cache);
    ...
    sanitizeUrlLikeFields(submitArgs);

    logs.push(`[submit.request] ${formatArgsForLog(submitArgs)}`);

    const submitResult = await callTool(session, meta.tools.submit.name, submitArgs);
